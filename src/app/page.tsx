...
export default async function Home() {
  ...
  if (session?.user) redirect("/recommendations");
  ...
  return (
    <main className="cinema-shell relative isolate flex min-h-svh flex-col overflow-hidden bg-background text-foreground">
      ...
      <header className="relative z-20 flex w-full items-center justify-between border-b border-white/8 px-5 py-5 sm:px-8 lg:px-12">
        ...
        <Link
          href="/login"
          className="rounded-full border border-white/10 px-4 py-2 text-[10px] font-medium tracking-[0.18em] text-stone-400 uppercase transition hover:border-amber-100/25 hover:text-stone-100 sm:text-[11px]"
        >
          Sign in
        </Link>
      </header>
      ...
      >
        <div className="hero-copy flex flex-col items-start lg:pb-10">
          ...
          <div className="mt-8 flex w-full flex-col gap-4 sm:mt-10 sm:w-auto sm:flex-row sm:items-center">
            <Link
              href="/signup"
              className="inline-flex h-12 items-center justify-center gap-2 rounded-full bg-stone-100 px-6 text-[11px] font-medium tracking-[0.16em] text-stone-950 uppercase shadow-[0_10px_50px_rgba(255,255,255,0.08)] transition hover:bg-amber-50"
            >
              Get started
            ...
            <Link
              href="/login"
              className="text-center text-[10px] tracking-[0.18em] text-stone-500 uppercase transition hover:text-stone-200 sm:text-left"
            >
